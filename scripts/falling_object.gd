...
func _ready() -> void:
	...
	# 2. Disable collisions while hovering
	collision_layer = 0
	collision_mask = 0
...
func _on_body_entered(_body: Node) -> void:
	...
	if can_play_sound and is_instance_valid(collision_player):
		...
		if is_first_impact:
			# --- HIT 1: Main Drop Impact (Full 100% Volume / 0 dB) ---
			...
			# --- HIT 2+: Secondary Bounces (Gated & Velocity Scaled) ---
			# Maps current speed smoothly from 0.10 (very quiet tap) to 0.25 (modest tap)
