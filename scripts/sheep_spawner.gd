...
func populate_sheep(container: Node2D, total_walls: int, dynamic_max_x: float) -> void:
	...
	var newly_added_sheep_ids: Array[int] = []

	# 1. If we have more walls than saved sheep, create and register the new sheep
	while saved_sheep_list.size() < target_count:
		...
		if temp_sheep.has_method("generate_random_color"):
		...
		var new_sheep_entry = {
			...
			"color": random_col.to_html(false) # e.g. "ffeedd"
		}
	...
	# 2. Spawn instances for every sheep in saved data
...
# 3. If new sheep were spawned this session, pass the entire array to the popup
